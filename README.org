...
- =nelix-manifest.el= desired-state workflow:
  ...
  =nelix-lock-write=, =nelix-lock-check=, and manifest-aware
  =nelix-upgrade-plan=.  This is the first Nix/Guix-like manifest
...
- =nelix-backend.el= / =nelix-store.el= / =nelix-registry.el= native
  skeleton: backend capabilities and policy selection, Nix-free
  =nelix-native-audit=, metadata-only native store/profile generations,
  rollback, profile prune/remove, and local registry list/search/show with
  fixture recipes.  This
  starts the path toward Windows/macOS/Nix-free operation.
...
- =bin/nelix= and =scripts/nelix-cli.el= provide the initial command
  ...
  [--prune]=, =prune-plan=, =lock=, =upgrade-plan=, =outdated=,
  =upgrade=, =list=, =rollback=, and =doctor=.  =--json= emits
