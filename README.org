...
- /Real-nix smoke test/ runs on every push (Phase 4-H, =smoke.yml=):
  - =make smoke-eval= renders four examples
    ...
    (Determinate Nix on =ubuntu-latest=).
  - =make smoke-build= (local-only) actually =nix build= s the
    cheap subset (=gnu-hello= + =black=) end-to-end.
  - Real source / cargo / vendor hashes ship in the recipes —
    no =sha256-PLACEHOLDER= literals to swap out before install.
