...
Two routes.  Install the Debian package if you want to *use* Nelix;
clone the repository if you want to work *on* it.  Both give you the
same bundled registry.

** Debian / Ubuntu package

#+begin_src shell
sudo dpkg -i elpa-nelix_0.1.0-5_all.deb
#+end_src

Nothing else is needed: =emacsen-common= puts Nelix on =load-path= and
byte-compiles it for each installed Emacs flavour.
...
*A locally built Emacs will not see it.*  The package reaches
=load-path= through =/etc/emacs/site-start.d=, which is distro
machinery; an Emacs built from source and installed under =/usr/local=
does not read it.  If =command -v emacs= points at such a build,
=(require 'nelix)= fails there while succeeding under
=/usr/bin/emacs=.  Add the directory yourself in that case:

#+begin_src elisp
(dolist (dir (file-expand-wildcards "/usr/share/emacs/site-lisp/elpa/nelix-*" t))
...
#+begin_src shell
git clone https://github.com/zawatton/nelix ~/src/nelix
cd ~/src/nelix
make check        # lint + ERT + schema/render/CLI smokes; no Nix needed
#+end_src

Point Emacs at the checkout:

#+begin_src elisp
(add-to-list 'load-path "~/src/nelix")
(require 'nelix)
(require 'nelix-dsl)
;; Register the pkg-* MCP tools (requires anvil.el loaded):
(nelix-core-enable)
#+end_src

=add-to-list= prepends, so if the Debian package is also installed,
adding the checkout *after* it is what makes the checkout win.  Nelix
then resolves its registry from the checkout as well — see below — so a
recipe you add under =registry/packages/= is live without reinstalling
anything.

The CLI runs straight from the tree:
...
Or install the wrapper without touching system directories:
...
Working on Nelix itself, the useful gates are:

#+begin_src shell
make check                             # the everyday gate
make compile                           # byte-compile, warnings as errors
make deb-local-gate                    # build the .deb and verify its contents
make verify-source-operational-gate    # the real manifest against this checkout
make verify-installed-operational-gate # the same against the installed package
#+end_src

=verify-installed-operational-gate= tests a *distro-installed* Nelix,
so it runs =/usr/bin/emacs= by default; set =EMACS= to override.

** The bundled registry

Both routes ship the same recipes — 200 Emacs packages, 6 system
commands and 1 library:

#+begin_src shell
bin/nelix --json registry list | head
#+end_src

#+begin_src elisp
(require 'nelix-registry)
(nelix-registry-update)
(length (nelix-registry-list))        ; => 207
(nelix-registry-packaged-root)        ; where they were found
#+end_src

=nelix-registry-packaged-root= follows the loaded library, so a
checkout reports its own =registry/= and an installed package reports
=/usr/share/emacs/site-lisp/elpa/nelix-*/registry=.  That is how a
checkout picks up recipes you are editing.

This is a starter archive, not a curated index: the Emacs recipes were
generated from one user's init and cover widely used packages.  Add
your own under =registry/packages/emacs/=, point
=nelix-registry-roots= at another directory, or subscribe to a remote
index — see below.

** Subscribing to a remote registry

A registry can be served as static files; there is no server side.
Point Nelix at an index and it fetches the recipes named there:

#+begin_src elisp
(setq nelix-registry-remotes
      '((:name "zawatton"
         :url "https://zawatton.github.io/nelix/index.el"
         ;; The publisher's index hash.  `make pages' prints it; a
         ;; literal here would be wrong the first time a recipe is added.
         :sha256 "sha256-...")))

(nelix-registry-update)
#+end_src

Two hashes are checked, not one.  You pin the index, and the index pins
every recipe it names, so neither can be swapped after you subscribed.
A changed byte anywhere is refused rather than installed.

The pinned hash is a contract: when the publisher issues a new index,
you update =:sha256= to match.  That is the cost of running unsigned.
Signing removes the step — a subscriber trusts a key instead of a
hash — through =nelix-registry-require-signature=,
=nelix-registry-trusted-signers= and =nelix-registry-public-keys=.

Remote recipes are cached locally, so a publisher who later turns
hostile cannot replace content you already hold.

To use *only* a remote and ignore what shipped with Nelix:

#+begin_src elisp
(setq nelix-registry-include-packaged-root nil)
#+end_src

** Publishing a registry

#+begin_src shell
make pages          # writes public/index.el plus the recipes it names
#+end_src

The target prints the index SHA-256; that is what subscribers pin.
Serve =public/= from any static host.  For GitHub Pages, publish it on
an orphan =gh-pages= branch so generated files stay out of the source
history — in a separate worktree, so the tree you are working in is
never emptied:

#+begin_src shell
make pages
git worktree add --orphan -b gh-pages ../nelix-gh-pages
cp public/index.el ../nelix-gh-pages/
cp -r public/packages ../nelix-gh-pages/packages
touch ../nelix-gh-pages/.nojekyll    # serve .el files as-is
git -C ../nelix-gh-pages add -A
git -C ../nelix-gh-pages commit -m "publish registry index"
git -C ../nelix-gh-pages push -u origin gh-pages
git worktree remove ../nelix-gh-pages
#+end_src

Check the tree before publishing it — the gate takes a directory:

#+begin_src shell
tools/nelix-registry-pages-gate.sh ../nelix-gh-pages
#+end_src

Then point Pages at the =gh-pages= branch root in the repository
settings.

Do not run =git checkout --orphan= in your working tree for this: the
follow-up =git rm -rf= would also take untracked build output with it
(=debian/.debhelper/=, =.cache/=, anything else not committed).

Publish deliberately, not on every push: each new index invalidates the
hash subscribers pinned, so automating it on commit would break them
continuously.

#+begin_src shell
make smoke-registry-pages   # subscribe to the tree you just built, and
                            # confirm a tampered recipe is refused
#+end_src

** Installing without Nix

The =nelix-native= backend builds into Nelix's own store and needs no
Nix at all:

#+begin_src elisp
(nelix-install "dash" :backend-policy '(nelix-native))
#+end_src

A manifest can declare the same thing once:

#+begin_src elisp
(nelix-manifest
 :name "my-emacs"
 :backend-policy '(nelix-native)
 :emacs '(dash s f magit))
#+end_src

Source builds need the usual tools on =PATH= (=git=, =curl=, =tar=,
=make=, a C compiler); a package that needs a C library can depend on
one built the same way, as =emacs-ffi= does with =libtool=.
...
make test       # ERT suite, no nix binary required (all mocked)
...
- Emacs 29+
- For source builds with =nelix-native=: =git=, =curl=, =tar=, =make=
  and a C compiler on =PATH=.  No Nix.
- For the =nix= backend: Nix 2.18+ with flakes enabled
- =anvil.el= loaded, if you want the =pkg-*= MCP tools
