...
*Phase 1+2+3+4-A+4-B+4-C+4-D+4-E+4-F SHIPPED + REAL-NIX VERIFIED* (2026-05-06) —
...
upstream MELPA recipe lookup, and a one-shot =async-installer=
importer that auto-populates =:depends-on= from local clones.

- ERT 91/91 PASS (mocked, runs without a nix binary).
...
#+begin_src shell
make test       # 91 ERT tests, no nix binary required (all mocked)
