...
*Phase 1+2 SHIPPED* (2026-04-27) — Full DSL stack on top of the Nix
store: =pkg-define= macro + Guix-style declarative records, pure-Elisp
=anvil-pkg-render-nix= renderer, registry + flake.nix generator,
=pkg-install= dispatching nixpkgs strings *and* declared symbols
through the same entry point.  ERT 12/12 PASS (mocked, no nix
binary needed to run the suite).
...
Phase 3 (Git-host fallback + additional source/build-system types)
is the next milestone; see =docs/design/02-dsl.org=.
...
| 3     | Git-host fallback + more source / build-system types                 | next           |
| 4     | Profile / generation management (rollback wrap)                      | planned        |
| 5+    | Optional: independent package server, OS angle                       | research       |
