...
*Phase 1+2+3 SHIPPED* (2026-04-27) — Full DSL stack on top of the
Nix store with multiple fetcher and build-system support.  ERT
21/21 PASS (mocked, no nix binary needed to run the suite).
...
Phase 4 (profile generation rollback) is the next milestone;
