...
*Phase 1+2+3+4-A+4-B+4-C SHIPPED + REAL-NIX VERIFIED* (2026-05-04) —
...
=:depends-on= derivation from =Package-Requires= via a small
raw.githubusercontent.com pre-fetch with per-session caching,
profile generation rollback (=pkg-list-generations= / =pkg-rollback=
/ =pkg-history=), Nix 2.34 =install=→=add= dispatch, and a one-shot
=async-installer= importer that auto-populates =:depends-on= from
local clones.

- ERT 55/55 PASS (mocked, runs without a nix binary).
...
Phase 4-D candidates: =melpaBuild= recipe synthesis (Phase 4-B OQ7),
url-fetch / git-fetch coverage of the L18 pre-fetch path,
per-package rollback granularity (Phase 4-C OQ13), and
=anvil-pkg-state= integration so the deps cache + Nix-version cache
+ generations mirror persist across Emacs sessions.  Phase 5
focuses on the NeLisp async backend that makes
=anvil-pkg-compat-make-process-async= work outside Emacs.
...
| 4-D   | melpaBuild recipe synthesis + url/git L18 + per-pkg rollback + state   | next           |
| 5+    | NeLisp async backend (replaces Phase 4-C compat-async stub)            | research       |
...
#+begin_src shell
make test       # 55 ERT tests, no nix binary required (all mocked)
